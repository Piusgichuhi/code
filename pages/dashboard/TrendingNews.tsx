...
type Topic = 'Cybersecurity' | 'AI' | 'DevOps' | 'AWS';
...
const TrendingNews: React.FC = () => {
    ...
    const [activeTopic, setActiveTopic] = useState<Topic>('Cybersecurity');
    ...
    useEffect(() => {
        const fetchNews = async () => {
            try {
                ...
                setArticles([]);
                setSources([]);
                const { articles: fetchedArticles, sources: fetchedSources } = await getTrendingNews(activeTopic);
                if (fetchedArticles.length === 0) {
                    setError(`Failed to fetch news for ${activeTopic}.`);
                } else {
    ...
    const topics: Topic[] = ['Cybersecurity', 'AI', 'DevOps', 'AWS'];

    return (
        <div className="p-8">
            <h2 className="text-3xl font-bold mb-6 text-gray-800 dark:text-white">Trending News</h2>
            ...
            {loading && <LoadingSpinner text={`Fetching latest ${activeTopic} news...`} />}
